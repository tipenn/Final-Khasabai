    ...
    <div class="container">
        <div class="row">
            <div class="col-4">
                ...
                <button type="submit" class="deletion">BACK</button></a>
            </div>
            <div class="col">
        ...
        <div class="col">
            ...
            <img src="assets/<?php echo $row["item_image"] ?>">
                ...
                <form method="POST">
    <center>
        <button type="submit" name="delete_item_<?php echo $row["item_code"]; ?>" class="delete_item">
...
<?php
if (isset($_POST['delete_item_' . $row["item_code"]])) {
    $itemCode = $row['item_code'];

    $query = "DELETE FROM products WHERE item_code = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('i', $itemCode); 
    $stmt->execute();
    $stmt->close();

    echo "<script>alert('Item successfully deleted!! Please reload to see changes~~');</script>";
}
?>
            </div>
        </div>
